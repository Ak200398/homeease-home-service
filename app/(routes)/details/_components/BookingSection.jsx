...
function BookingSection({ children, business }) {
    ...
    useEffect(() => {
        // Initialize date only on the client
        setDate(new Date());
        getTime();
    }, []);
    ...
    const BusinessBookedSlot = () => {
        if (!business.id) return; // Ensure business ID is available
    ...
    const saveBooking = () => {
        if (!date || !selectedTime || !data) return; // Ensure all required data is available
        GlobalApi.createNewBooking(business.id,
            moment(date).format('DD-MMM-yyyy'), selectedTime, data.user.email, data.user.name)
            .then(resp => {
                console.log(resp);
                if (resp) {
                    setDate(new Date()); // Reset to current date or desired default
                    setSelectedTime('');
